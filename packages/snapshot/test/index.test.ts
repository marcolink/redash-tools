...
describe('A querySnapshot function', () => {
    it('calculate the right url', async () => {
        await querySnapshot({token: '123', queryId: 'qId', visualizationId: 'vId'})
        expect(stubPage.goto).toHaveBeenCalledWith(`https://redash.io/embed/query/qId/visualization/vId?api_key=123`, {"waitUntil": "networkidle0"})
    });
});
