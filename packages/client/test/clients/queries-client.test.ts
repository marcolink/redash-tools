...
describe('A queries client', () => {
  ...
  describe('with a "one" function', () => {
    ...
    it('fires a request with default parameters', async () => {
      ...
      expect(fetch).toHaveBeenCalledWith(
        'https://redash.io/api/queries/test-id',
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
  ...
  describe('with a "cachedResult" function', () => {
    ...
    it('fires a result request', async () => {
      ...
      expect(fetch).toHaveBeenCalledWith(
        'https://redash.io/api/queries/result-id/results',
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
  ...
  describe('with a "updatedResult" function', () => {
    it('returns up-to-date result', async () => {
      ...
      const result = await client.getUpdatedResult({
        id: 'query-id',
        max_age: 1000,
      });
      expect(result).toEqual(mockResultResponse);
    ...
    it.skip('wait polls and returns updated result', async () => {
      ...
      when(fetchMock)
        .calledWith(
          'https://redash.io/api/queries/query-id/results?max_age=0',
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
        ...
        .mockReturnValue(
          createMockResponse(createJobMock({ status: 3, query_result_id: 23 }))
        )

        .calledWith(
          'https://redash.io/api/jobs/fb41ef5f-57a1-46b5-ada2-043dc57ffa38',
          {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
